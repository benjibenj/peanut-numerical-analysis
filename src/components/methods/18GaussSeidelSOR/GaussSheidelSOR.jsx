...
import gaussSheidelSORunction from "./GaussSheidelSORunction";
...
const GaussSheidelSOR = ({ name }) => {
  ...
  useEffect(() => {
    ...
    if (matrixA.length !== 0 && B.length !== 0) {
      setResults(gaussSheidelSORunction(matrixA, B));
    }
  }, [matrixA, B]);
  return (
    <Method
      title={name}
      prev={methods.find(method => method.index === 17)}
...
export default GaussSheidelSOR;
