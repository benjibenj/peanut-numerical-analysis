...
import gaussSheidelFunction from "./GaussSheidelFunction";
...
const GaussSheidel = ({ name }) => {
  ...
  useEffect(() => {
    ...
    if (matrixA.length !== 0 && B.length !== 0) {
      setResults(gaussSheidelFunction(matrixA, B));
    }
  }, [matrixA, B]);
  return (
    <Method
      ...
      next={methods.find( method => method.index === 18)}
    >
      <Inputs>
